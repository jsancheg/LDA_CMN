...
mod_CCR0 <- gls(CCR ~ Variables, data = ssdf,
...
colnames(ssdf)
mod_CCR50 <- gls(CCR ~   Variables + Number_Classes + Number_Separating_Variables + Number_Variables+  
...
mod_CCR50 <- gls(CCR ~   as.factor(Number_Classes)*Variables + 
                 Group_Mean_Distance * Variables+ Class_Proportion*Variables + 
                 Covariance_Structure*Variables + as.factor(Number_Variables)*Variables ,
               data = ssdf,
               correlation = corSymm(form = ~ 1|Nsim))
...
summary(mod_CCR1)
summary(aov(mod_CCR1))

qqnorm(resid(mod_CCR))
