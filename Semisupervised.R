...
add_diff_to_wider <- function(df)
{
  ...
  metrics_na_0 <- metrics_na_0 %>% mutate(Replicate = Nsim)
  metrics_na_0 <- metrics_na_0 %>% mutate(Nsim = row_number())
  metrics_na_0 <- metrics_na_0 %>% relocate(File, .before = Nsim)
  metrics_na_0 <- mutate(metrics_na_0, Model_Selected = Model_SM)
  
  
  aux_df0 <- metrics_na_0 %>% mutate(
    IncludeX2                          = as.numeric(str_detect(Model_SM,"X2")),
    IncludeX4                          = as.numeric(str_detect(Model_SM,"X4")),
    IncludeX5                          = as.numeric(str_detect(Model_SM,"X5")),
    Number_Classes                     = as.numeric(str_split(File,"_",simplify = TRUE)[,2]),
    Number_Separating_Variables        = as.numeric(str_split(File,"_",simplify = TRUE)[,3]),
    Number_Variables                   = as.numeric(str_split(File,"_",simplify = TRUE)[,4]),
    Number_Observations                = as.numeric(str_split(File,"_",simplify = TRUE)[,5]),
    Training_Proportion                = as.numeric(str_split(File,"_",simplify = TRUE)[,6]),
    Class_Proportion                   = str_split(File,"_",simplify = TRUE)[,7],
    Covariance_Structure               = str_split(File,"_",simplify = TRUE)[,8],
    Group_Mean_Distance                = str_split(File,"_",simplify = TRUE)[,9],
    AlphaC                             = str_split(File,"_",simplify = TRUE)[,10],
    EtaC                               = str_split(File,"_",simplify = TRUE)[,11],
    ...
    diffSpecificity_class_all_tm       = Specificity_SaturatedM - Specificity_TM,
    diffSpecificity_class_sm_all       = Specificity_SM - Specificity_SaturatedM,
    diffPrecision_class_sm_tm          = Precision_SM - Precision_TM,
    diffPrecision_class_all_tm         = Precision_SaturatedM - Precision_TM,
    diffPrecision_class_sm_all         = Precision_SM - Precision_SaturatedM,
    diffF1_class_sm_tm                 = F1_SM - F1_TM, 
    diffF1_class_all_tm                = F1_SaturatedM - F1_TM,
    diffF1_class_sm_all                = F1_SM - F1_SaturatedM,
    diffCCRV_sm_tm                     = CCRCont_SM - CCRCont_TM,
    diffCCRV_all_tm                    = CCRCont_SaturatedM - CCRCont_TM,
    diffCCRV_sm_all                    = CCRCont_SM - CCRCont_SaturatedM,
    diffSensitivityV_sm_tm             = RecallV_SM - RecallV_TM,
    diffSensitivityV_all_tm            = RecallV_SaturatedM - RecallV_TM,
    diffSensitivityV_sm_all            = RecallV_SM - RecallV_SaturatedM,
    diffSpecificityV_sm_tm             = SpecificityV_SM - SpecificityV_TM,
    diffSpecificityV_all_tm            = SpecificityV_SaturatedM - SpecificityV_TM,
    diffSpecificityV_sm_all            = SpecificityV_SM - SpecificityV_SaturatedM,
    diffPrecisionV_sm_tm               = PrecisionV_SM - PrecisionV_SaturatedM,
    diffPrecisionV_all_tm              = PrecisionV_SaturatedM - PrecisionV_TM,
    diffPrecisionV_sm_all              = PrecisionV_SM - PrecisionV_SaturatedM,
    diffF1V_sm_tm                      = F1V_SM - F1V_TM,
    diffF1V_all_tm                     = F1V_SaturatedM - F1V_TM,
    diffF1V_sm_all                     = F1V_SM - F1V_SaturatedM
  )
  
  aux_df0$Model_TM[aux_df0$Number_Separating_Variables == 2 & aux_df0$Model_TM == ""] <- "X2-X4"
  aux_df0$Model_TM[aux_df0$Number_Separating_Variables == 3 & aux_df0$Model_TM ==""] <- "X2-X4-X5"
  
  
  aux_df0$Nvars_TM[aux_df0$Number_Separating_Variables == 2 & aux_df0$Nvars_TM ==0] <- 2
  aux_df0$Nvars_TM[aux_df0$Number_Separating_Variables == 3 & aux_df0$Nvars_TM ==0] <- 3
  
  
  aux_df0 <- aux_df0 %>% mutate(Model_TM = sapply(Model_TM, sort_labels)) 
  aux_df0 <- aux_df0 %>% mutate(Model_SM = sapply(Model_SM, sort_labels)) 
  aux_df0 <- aux_df0 %>% mutate(Model_SaturatedM = sapply(Model_SaturatedM, sort_labels)) 
  
  # saveRDS(aux_df0,"sMetrics_wider_with_dif_2024_03_18.RDS")
...
SemiSupervised_GS_General <- function(file_name,pathScenarios,CE, variables_True_Model, 
                              pnolabeled = 0.5, niterations = 10,
                              alpharef = 0.99, tol = 0.01, epsilon = 0)
  # Semisupervised_HLS: function for semi-supervised fitting with head long search algorithm
{
  ...
  for (i_sim in 1:nsimulations)
  {
    ...
    # nolebeled: contains the number of unlabeled
    nolabeled <- floor(pnolabeled*ntrain)
    if(length(ind_nolabeled) !=0)
    {
      ind_nolabeled <- sample(1:ntrain,nolabeled,replace = FALSE)
    }else ind_nolabeled = NULL
