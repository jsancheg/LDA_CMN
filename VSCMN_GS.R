...
fHLvarSearch2_GS <- function(X_train, X_test, RW,l_train, l_test, CE, 
...
contDf <- function(X,y,lab,vpi,alpha,eta,ptrain,ns = 100, iterations = 20)
{
  ...
  for (i in 1:ns)
  {
    ...
    # original line change for an update
...
#    pred_cont_Sat_M <- 1-sapply(1:length(DfTestl),function(j) { saturated_mod$predv[j,DfTestl[j]] })
    ...
    pred_cont_Sat_M <- saturated_mod$vtest_hat     
...
#    }
    ...
    # original line changed for an update
