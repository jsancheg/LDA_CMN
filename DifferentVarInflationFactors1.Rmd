...
sigma_a[,,1] <- diag(c(1.5,3,3),p_a)
alpha_a <- 0.8
eta_a <- 5
DatasetA10 <- SimGClasses(mu_a,sigma_a,pig_a,nobs_a,ptraining,alpha_a,eta_a)
saveRDS(DatasetA10,"DatasetA10.RDS")
DatasetA <- readRDS("DatasetA10.RDS")
Model <- c("EEI","VEI")
fit_AEq <- CNmixt(X = DatasetA$Xtrain,G = G_a, contamination = TRUE, model = Model,
...
ms_ADifrealv <- mCmn_DIF_VVV(Xtrain = DatasetA$Xtrain,ltrain = DatasetA$ltrain, par = par_Areal)
...
#sigma_ArealDif <- as.vector(ms_ADifrealv$lambda)*ms_ADifrealv$sigma
sigma_ArealDifr<- round(matrix(unlist(ms_ADifrealv$sigma),ncol=p_a,nrow=p_a,byrow = TRUE ),2)
...
# passing the parameters obtained by the equal inflation factor model as initial parameters
...
fit_ADif <- emCmn_DIF _VVV(X = DatasetA$Xtrain,labels = DatasetA$ltrain, par0, 
...
ms_BDifrealv <- mCmn_DIF_VVV(Xtrain = DatasetB$Xtrain,ltrain = DatasetB$ltrain, par = par_Breal)
...
sigma_BDifr<- round(fit_BDif$sigma,2)
...
The models considered assume independence among the observed variables. Therefore $\Sigma_{g}$ is diagonal matrix.
...
b) A contaminated dataset of $300$ samples in $p=3$ dimensions with different variable inflation factor within group and $G=1$;
...
$$
\mu = `r write_matex2(mu_a)` , \Sigma = `r write_matex2(sigma_a)`, \alpha = `r format(alpha_a,digits = 2)` , \eta = `r format(eta_a,digits = 2)`
$$
...
pairs(DatasetA$Xtrain, panel = function(x,y, ...) {
  points(x,y, 
         col = ifelse(cond1Areal_trainv == "11"|
                        cond1Areal_trainv == "00","blue","red"), 
#           c("red","blue")[DatasetA$vtrain + 1],
...
$$
\mu = \begin{pmatrix} 0 \\ 0 \\ 0\\  \end{pmatrix} , \Sigma = I_{3} = \begin{pmatrix} 1 & 0 & 0  \\ 0 & 1 & 0  \\ 0 & 0 & 1   \\ \end{pmatrix}, \alpha = `r format(alpha_b,digits=2)`, \dot{N}_{g} = \begin{pmatrix} 1 & 0 & 0 \\ 0 & 5 & 0  \\ 0 & 0 & 1  \\  \end{pmatrix}, {N}_{g} = \begin{pmatrix} 1 & 0 & 0 \\ 0 & 25 & 0  \\ 0 & 0 & 1  \\  \end{pmatrix}
$$
