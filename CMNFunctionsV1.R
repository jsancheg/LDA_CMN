...
setwd(work_path)
source("utilities.R")
path_output <- "/home/jsancheg/git_environment/LDA_CMN/Scenarios/"
dir(path_output)
...
CreateSimFiles <- function(F1,F2,F3,F4,F5,F6,F7,F8,F9,F10,pathOutput)
  # Create all possible combinations of simulated datasets
  # F1 : Mean distance
  # F2 : Number of classes
  # F2mu: Array when the number of columns is equals to F5 and the rows is equals to F2
  # F1G : Array of three dimensions pxpxG where p is the number of variables F5 and the third dimention
  ...
  # F7ij: array of the pair to be correlated
  # F7Corr: correlation value
  ...
  # F10: number of separating variables
{
  ...
  Sets <- as.matrix(expand.grid(F2,F10,F5,F4,F3,F1,F7,F6,F8,F8,F9,F9))
  Sets
  nrow(Sets)
  i <- 1
  Sets[1,]
  ...
  for(i in 1:1) # first 10 cases of c
  {
    for(i_run in 1: nruns)
    {
      ...
      f9 <- c(as.numeric(Sets[i,11]),as.numeric(Sets[i,12])) # eta's
      GenData[[i_run]] <- SimGClasses(mu,sg,f3,f4,f6,f8,f9)
      
    } # End-for replicates
    dfname <- paste0("S_",f2,"_",f10,"_",f5,"_",
                     ...
                     as.numeric(Sets[i,11]), as.numeric(Sets[i,12]))
    dfname
    saveRDS(GenData,paste0(path_output,dfname,"_",i_run,".RDS"))
    cat("\n writing file: ",paste0(dfname,"_",i_run,".RDS") ,"\n")
