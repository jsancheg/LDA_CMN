...
MetricsDf <- MetricsDf %>% mutate(Number_of_Variables = 
                                    str_split(MetricsDf$Setting,"_",simplify = TRUE)[,4])
...
MetricsDf$Number_of_Variables <- as.factor(MetricsDf$Number_of_Variables)

auxDf1 <- MetricsDf %>% dplyr::filter(Number_of_Variables == 100) %>% 
  dplyr::select(Setting,Simulation,AccuracyTM,AccuracySM,AccuracySaturatedM,
...
# "OutputS_2_3_100_BAL_SCBSV_VO"
VODf1 <- MetricsDf1 %>% dplyr::filter(Group_Mean_Distance == "VO",
                               Setting == "OutputS_2_3_100_5050_SCBSV_VO")
class(VODf1$Variables)


mean2_3_100_BAL_SCBSV <- VODf1 %>% dplyr::group_by(Variables) %>%
  summarise(mean.accuracy  = mean(Accuracy_class))
mean2_3_100_BAL_SCBSV


g2_3_100_BAL_SCBSV <- ggplot(VODf1, aes(x = Group_Mean_Distance, y = Accuracy_class, 
                          color = Variables)) + 
  geom_boxplot_pattern(pattern_color = "white",
                       pattern_fill = "black",
                       aes(pattern= Variables))+
  ylab("Test Accuracy") + xlab("SCBSV") + ylim(0.1,1)
...
VODf1 <- MetricsDf1 %>% filter(Group_Mean_Distance == "VO",
                               Setting == "OutputS_2_3_100_BAL_SCBSNSV_VO")

mean2_3_100_BAL_SCBSNSV <- VODf1 %>% dplyr::group_by(Variables) %>%
  summarise(mean.accuracy  = mean(Accuracy_class))
mean2_3_100_BAL_SCBSNSV


g2_3_100_BAL_SCBSNSV <- ggplot(VODf1, aes(x = Group_Mean_Distance, y = Accuracy_class, 
                                         color = Variables)) + 
  geom_boxplot_pattern(pattern_color = "white",
                       pattern_fill = "black",
                       aes(pattern= Variables))+
  ylab("Test accuracy") + xlab("SCBSNSV") + ylim(0.1,1)


g2_3_100_BAL_SCBSNSV
...
VODf1 <- MetricsDf1 %>% filter(Group_Mean_Distance == "VO",
                               Setting == "OutputS_2_3_100_BAL_SCBNSV_VO")

mean2_3_100_BAL_SCBNSV <- VODf1 %>% group_by(Variables) %>%
  summarise(mean.accuracy  = mean(Accuracy_class))
mean2_3_100_BAL_SCBNSV


g2_3_100_BAL_SCBNSV <- ggplot(VODf1, aes(x = Group_Mean_Distance, y = Accuracy_class, 
                                          color = Variables)) + 
  geom_boxplot_pattern(pattern_color = "white",
                       pattern_fill = "black",
                       aes(pattern= Variables))+
   ylab("Test accuracy") + xlab("SCBNSV") + ylim(0.1,1) +
...
VODf1 <- MetricsDf1 %>% filter(Group_Mean_Distance == "VO",
                               Setting == "OutputS_2_3_100_BAL_IND_VO")

mean2_3_100_BAL_IND <- VODf1 %>% group_by(Variables) %>%
  summarise(mean.accuracy  = mean(Accuracy_class))
mean2_3_100_BAL_IND


g2_3_100_BAL_IND <- ggplot(VODf1, aes(x = Group_Mean_Distance, y = Accuracy_class, 
                                          color = Variables)) +
  geom_boxplot_pattern(pattern_color = "white",
                       pattern_fill = "black",
                       aes(pattern= Variables))+
  ylab("Test accuracy") + xlab("IND") + ylim(0.1,1) +
  theme(axis.text.x = element_blank())
...
# "OutputS_2_2_100_5050_IND_VO"    All (0.80), Selected (0.82), True (0.81) are similar
...
mean_2_2_100_BAL <- rbind.data.frame(mean2_2_100_BAL_IND,
                                       mean2_2_100_BAL_SCBNSV,
                                       mean2_2_100_BAL_SCBSNSV,
                                       mean2_2_100_BAL_SCBSV) 
mean_2_2_100_BAL %>% group_by(Variables) %>%   
...
F1meanS_2_2_100_UNBAL_SCBNSV <- VODf1 %>% group_by(Variables) %>%
  summarise(mean.F1  = mean(F1_Class))
F1meanS_2_2_100_UNBAL_SCBNSV
...
F1meanS_2_2_100_UNBAL_IND <- VODf1 %>% group_by(Variables) %>%
  summarise(mean.F1  = mean(F1_Class))
F1meanS_2_2_100_UNBAL_IND
...
# 2 balanced groups 
gr10 <- ggarrange(  g2_2_100_BAL_IND,
                    g2_2_100_BAL_SCBNSV,
                    g2_2_100_BAL_SCBSNSV,
                    g2_2_100_BAL_SCBSV,
                    ncol=2,nrow = 2,
...
# 3 balanced groups
gr12 <- ggarrange(g2_3_100_BAL_IND,
                    g2_3_100_BAL_SCBNSV,
                    g2_3_100_BAL_SCBSNSV,
                    g2_3_100_BAL_SCBSV,
                    ncol = 2, nrow = 2,
...
# 2 unbalanced groups
gr11 <- ggarrange(g2_2_100_UNBAL_IND,
                    g2_2_100_UNBAL_SCBNSV,
                    g2_2_100_UNBAL_SCBSNSV,
                    g2_2_100_UNBAL_SCBSV,
                    ncol=2,nrow = 2,
...
gr13 <- ggarrange(F1g2_2_100_UNBAL_IND,
                  F1g2_2_100_UNBAL_SCBNSV,
                  F1g2_2_100_UNBAL_SCBSNSV,
                  F1g2_2_100_UNBAL_SCBSV,
                  ncol=2,nrow = 2,
