...
#  eta_aux <- (diag(sum_eta) ) / ((  det(sum_eta) )^(1/p)   )
...
cem_dif_s_B_1 <- function(X, max_iter = 100, tol = 1e-3)
{
  ...
  eta <- as.vector(runif(p,min=1,max = 1000))
  
  for( iter in 1:max_iter)
  {
    # E-step
    pdf <- e_step_dif_s_B_1(X,mu,sigma,alpha,as.vector(eta) )
    sum(pdf)
    # CM-steps
    new_params <- cm_step_dif_s_B_1(X,l,pdf,as.matrix(mu),sigma,alpha,as.vector(eta) )
    ...
    # update parameters
      ...
      return(list(mu = mu, sigma = sigma, alpha = alpha, eta = as.vector(eta)) )
}
...
es_BDifrealv1 <- e_step_dif_s_B_1(X = Xtrain, mu = ms_BDifrealv$mu,
