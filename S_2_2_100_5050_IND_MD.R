...
MultSimPar <- function(nruns)
{
  ...
  for (i_runs in 1:nruns)
  {
    ...
    Inclusion_correctness[[i_runs]] <- sum(length(intersect(aux$CM,True_model)) == length(True_model))
    Number_var_incorrect_included[[i_runs]] <- length(setdiff(aux$CM,True_model)) 
    Exclusion_correctness[[i_runs]] <- sum(length(setdiff(aux$CM,True_model)) == 0)
