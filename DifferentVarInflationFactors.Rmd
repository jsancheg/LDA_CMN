...
# Giving as initial values for parameters the real values
par_BDif0 <- list(G = G_b, pig = pig_b, mu = mu_b, 
                sigma = sigma_b, 
                alpha = alpha_b,
                eta = eta_b)
...
pred_BDif0 <- eCmn_DIF(X = DatasetB$Xtest,labels = DatasetB$ltest, par = par_BDif0)

Accuracy_BDif0 <- Accuracy(pred_BDif0$vhat,DatasetB$vtest)
Sensitivity_BDif0 <- Sensitivity(DatasetB$vtest, pred_BDif0$vhat,positive = 0)
Specificity_BDif0 <- Specificity(DatasetB$vtest,pred_BDif0$vhat,positive = 0) 
Recall_BDif0 <- Recall(DatasetB$vtest, pred_BDif0$vhat,positive = 0) 
Precision_BDif0 <- Precision(DatasetB$vtest,pred_BDif0$vhat,positive = 0) 
ConfusionMatrix_BDif0 <- ConfusionMatrix(pred_BDif0$vhat,DatasetB$vtest)
ConfusionDF_BDif0 <- ConfusionDF(pred_BDif0$vhat,DatasetB$vtest)
F1_Score_BDif0 <- F1_Score(DatasetB$vtest,pred_BDif0$vhat,positive = 0)


# Giving the real values for eta and 

par_BDif0$v <-as.matrix( DatasetB$vtrain )

ms_BDif0 <- mCmn_DIF_EII(Xtrain = DatasetB$Xtrain,ltrain = DatasetB$ltrain, par = par_BDif0)
...
# as lambda is present in the model EII then we modify sigma to be multiplied 
# by lambda

par_BDif0 <- list(G = G_b, pig = pig_b, mu = mu_b, 
                sigma = lambda * sigma_b, 
                alpha = alpha_b,
                eta = eta_b)

loglikCMN_DIF(X = DatasetB$Xtrain,labels = DatasetB$ltrain , par = par_BDif0)
...
par_BDif0$v <- matrix(runif(nrow(DatasetB$Xtrain)*G_b), 
...
#  Runing the M-step when the labels are the true and estimate the parameters
