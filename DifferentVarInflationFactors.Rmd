...
#fit_ADif <- mCmn_DIF(Xtrain = DatasetA$Xtrain,ltrain = DatasetA$ltrain, par = par_ADif0)

#sigma_ADif <- matrix(unlist(fit_ADif$sigma),ncol = p_a,nrow = p_a,byrow = TRUE)
#eta_ADif <- sqrt(matrix(unlist(fit_ADif$eta),ncol = p_a,nrow =p_a,byrow = TRUE )) 
#par_ADif <- list(G = 1, pig = 1, mu = fit_ADif$mu, 
#                sigma = sigma_ADif, 
#                alpha = fit_ADif$alpha,
#                eta = eta_ADif)
...
#mu_ADifr <- round(fit_ADif$mu,2)
#sigma_ADifr<- round(sigma_ADif,2)
#alpha_ADifr <- round(fit_ADif$alpha,2)
#eta_ADifr <- round(fit_ADif$eta,2)
