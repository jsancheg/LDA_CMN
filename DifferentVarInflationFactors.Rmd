...
ms_ADifrealv <- mCmn_DIF_EII(Xtrain = DatasetA$Xtrain,ltrain = DatasetA$ltrain, par = par_Areal)
...
fit_ADif <- emCmn_DIF_EII(X = DatasetA$Xtrain,labels = DatasetA$ltrain, Maxiterations = 10,threshold = 0.01)


par_ADif <- NULL
par_ADif <- list(G = fit_ADif$G, pig = fit_ADif$pig, mu = fit_ADif$mu,
                 sigma = fit_ADif$lambda * fit_ADif$sigma,
...
indADif_T0_P0_Trainv <- which(condADif_00Trainv == TRUE)
...
#predBTrainv <- ifelse(fit_BEq$models[[1]]$v>0.5,1,0)
#FactpredBTrainv <- factor(predBTrainv, levels = c("0","1"),
#                         labels=c("Cont","Non-cont"))
#FactBTrainv <- factor(DatasetB$vtrain, levels = c("0","1"),
#                     labels = c("Cont","Non-cont"))

#CM_BTrainv <- ConfusionMatrix(FactpredBTrainv,FactBTrainv)

#CompBTrainv <- cbind(True = DatasetB$vtrain, Pred = predBTrainv)

# condition 1 True contaminated samples
#condTrueB_0Trainv <- CompBTrainv[,1] == 0
#condPredB_1Trainv <- CompBTrainv[,2] == 1

#indB_T0_P1_trainv <- condTrueB_0Trainv & condPredB_1Trainv

#CompBTrainv[indB_T0_P1_trainv,]
...
ms_BDifrealv <- mCmn_DIF_EII(Xtrain = DatasetB$Xtrain,ltrain = DatasetB$ltrain, par = par_Breal)
...
fit_BDif <- emCmn_DIF_EII(X = DatasetB$Xtrain,labels = DatasetB$ltrain, Maxiterations = 10, threshold = 0.1)


par_BDif <- NULL
par_BDif <- list(G = fit_BDif$G, pig = fit_BDif$pig, mu = fit_BDif$mu,
                 sigma = fit_BDif$lambda * fit_BDif$sigma,
...
indBDif_T0_P0_Trainv <- which(condBDif_01Trainv == TRUE)
...
pig_CEq <- fit_CEq$models[[1]]$prior
...
mu_CDifr <- round(fit_CDif$mu,2)
sigma_CDif <- as.vector(fit_CDif$lambda)*fit_CDif$sigma
sigma_CDifr<- round(as.vector(fit_CDif$lambda)*fit_CDif$sigma,2)
alpha_CDifr <- round(fit_CDif$alpha,2)
eta_CDifr <- round(fit_CDif$eta,2)

fit_CDif$lambda
fit_CDif$eta
    eta_CDif <- array(0.0, dim = c(p_c,p_c,G_c))

if(is.array(fit_CDif$eta) )
  if(all(dim(fit_CDif$eta)==c(p_c,p_c,G_c)) == TRUE )
  {
    for(g in 1:G_c)
        eta_CDif[,,g] <- (matrix(unlist(fit_CDif$eta[,,g]),ncol = p_c,nrow =p_c,byrow = TRUE )) 
  }
  
     par_CDif <- list(G = 1, pig = 1, mu = fit_CDif$mu,
                 sigma = sigma_CDif,
                 alpha = fit_CDif$alpha,
                 eta = eta_CDif )
...
pred_CDif <- eCmn_DIF(X = DatasetC$Xtest,labels = DatasetC$ltest, par = par_CDif)

Accuracy_CDif <- Accuracy(pred_CDif$vhat,DatasetC$vtest)
Sensitivity_CDif <- Sensitivity(DatasetC$vtest, pred_CDif$vhat,positive = 0)
Specificity_CDif <- Specificity(DatasetC$vtest,pred_CDif$vhat,positive = 0) 
Recall_CDif <- Recall(DatasetC$vtest, pred_CDif$vhat,positive = 0) 
Precision_CDif <- Precision(DatasetC$vtest,pred_CDif$vhat,positive = 0) 
ConfusionMatrix_CDif <- ConfusionMatrix(pred_CDif$vhat,DatasetC$vtest)
ConfusionDF_CDif <- ConfusionDF(pred_CDif$vhat,DatasetC$vtest)
F1_Score_CDif <- F1_Score(DatasetC$vtest,pred_CDif$vhat,positive = 0)

ConfusionMatrix_CDif
ConfusionMatrix_CEq


t_CEq <- as.data.frame.matrix(ConfusionMatrix_CEq)
colnames(t_CEq)

t_CDif <-as.data.frame.matrix(ConfusionMatrix_CDif)

t_C <- cbind(t_CEq,t_CDif)

df_c <- data.frame(
  Metric = c("Accuracy","Precision","Recall","Sensitivity","Specificity",
             "F1 score"),
  Equal_IF = c(round(Accuracy_CEq,2),round(Precision_CEq,2),
               round(Recall_CEq,2),round(Sensitivity_CEq,2),
               round(Specificity_CEq,2),
            round(F1_Score_CEq,2)),
  Different_IF = c(round(Accuracy_CDif,2),round(Precision_CDif,2),
                   round(Recall_CDif,2),round(Sensitivity_CDif,2),
                   round(Specificity_CDif,2),
            round(F1_Score_CDif,2))
)
...
pairs(DatasetA$Xtest, panel = function(x,y, ...) {
  points(x,y, 
         col = c("red","blue")[DatasetA$vtest + 1],
         pch = ifelse(DatasetA$vtest == 0,24,19),
         cex = 1,
         ...)
      text(x[indAreal_T0_P0_Testv],y[indAreal_T0_P0_Testv],
           labels=c(indAreal_T0_P0_Testv),pos = 4)
} )
```
...
pairs(DatasetB$Xtrain, panel = function(x,y, ...) {
  points(x,y, 
         col = c("red","blue")[DatasetB$vtrain + 1],
         pch = ifelse(DatasetB$vtrain == 0,24,19),
         cex = 1,
         ...)
      text(x[indBreal_T0_P0_Trainv],y[indBreal_T0_P0_Trainv],
           labels=c(indBreal_T0_P0_Trainv),pos = 4)
} )
```
It is possible to observed that in the training and test subset there is a much higher dispersion in the variable $X2$ in comparison with $X_{1}$ and $X_{3}$.
...
pairs(DatasetB$Xtest, panel = function(x,y, ...) {
  points(x,y, 
         col = c("red","blue")[DatasetB$vtest + 1],
         pch = ifelse(DatasetB$vtest == 0,24,19),
         cex = 1,
         ...)
#      text(x[indBreal_T0_P0_Testv],y[indBreal_T0_P0_Testv],
#           labels=c(indBreal_T0_P0_Testv),pos = 4)
} )
```

Similar procedure is carry out fitting the data with the model EII assuming same variable inflation factor in the group and the parameters obtained are:
...
$$
The parameters obtained for $\mu,\Sigma,$ and $\lambda$ are very similar as the initial values for the parameters come from the estimates for the model EII with same variable inflation factor within group. The main difference between the estimates produced by these two models lies in the estimates for $\alpha$ that is $0.51$ which means that the inflation factor is being overestimated as the percentage of non-contaminated samples is $0.9$. Also, It is possible to see that the variable inflation factor estimates are different for variables $X_{1},X_{2}$ and $X_{3}$. Also, the estimate for the variable inflation factor corresponding to $X_{2}$ is smaller that the true variable.
